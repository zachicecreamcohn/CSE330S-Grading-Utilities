...
def parse_repo_names_from_txt(module_number, module_type: ModuleType):
    ...
    repo_mapping_file_path = (
        f"./text-grader-mappings/module-{module_number}-{module_type.value}.txt"
    )
...
def find_grade_in_readme(readme_content, repo_url, module_type: ModuleType):
    ...
    if table_match:
        total_earned, total_possible = table_match.group(1), table_match.group(2)
    else:
        for line in reversed(readme_content.splitlines()):
            m = re.search(r"(\d+(?:\.\d+)?)\s*/\s*(\d+(?:\.\d+)?)", line)
            if m:
                total_earned, total_possible = m.group(1), m.group(2)
                break

    student_ids.update(re.findall(r"\b\d{6}\b", readme_content))

    if not (total_earned and total_possible):
...
def process_single_repo(repo, base_url, parsed_grades, module_type: ModuleType):
    ...
    try:
        ...
        if total_earned and total_possible and len(student_ids) > 0:
            ...
            print(
                f"[SUCCESS] Parsed grade: {total_earned}/{total_possible} for {full_repo_url}."
            )
        else:
...
def parallelize_grade_parsing(repo_names, base_url, module_type):
    ...
    with ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(
                process_single_repo, repo, base_url, parsed_grades, module_type
            )
